...
void setState(CoasterState newState) {
  ...
  switch(newState) {
    ...
    case STATE_NOT_AT_TILTDROP: currentStateName = "NOT_AT_TILTDROP"; break;
    case STATE_ENTER_TILTDROP: currentStateName = "ENTER_TILTDROP"; break;
    case STATE_DROP: currentStateName = "DROP"; break;
    case STATE_RESET_DROP: currentStateName = "RESET_DROP"; break;
...
bool LedWhiteFlash(int numFlashes, int intervalMs){
  ...
  if(now - localStart >= intervalMs){
    localStart = now;
    localFlashCount++;
    if(localFlashCount %2==1) setAllLeds(CRGB::White,255);
...
void callback(char* topic, byte* payload, unsigned int length){
  ...
  if(String(topic)=="tiltdrop/manual"){
    ...
    setState(STATE_IDLE);
    if(!manualMode) tiltdropMotorManual=false;
  }
  else if(String(topic)=="tiltdrop/motor/open" && manualMode){
    tiltTrackStepper.move(-STEPS_TILT_TRACK);
    tiltdropMotorState=true;
    tiltdropMotorMoving=true;
  }
  else if(String(topic)=="tiltdrop/motor/close" && manualMode){
    tiltTrackStepper.move(STEPS_TILT_TRACK);
    tiltdropMotorState=false;
    tiltdropMotorMoving=true;
  }
  else if(String(topic)=="releasedrop/motor/open" && manualMode){
    for(int pos=90;pos>=0;pos--) releaseServo.write(pos);
    releasedropMotorState=true;
  }
  else if(String(topic)=="releasedrop/motor/close" && manualMode){
...
void connectMQTT(){
  while(!client.connected()){
    ...
    if(client.connect("tiltdropESP32")){
      ...
      client.subscribe("tiltdrop/motor/open");
      client.subscribe("tiltdrop/motor/close");
      client.subscribe("releasedrop/motor/open");
      client.subscribe("releasedrop/motor/close");
    }else{
...
void loop(){
  ...
  if(manualMode){
    if(tiltdropMotorManual) tiltTrackStepper.runSpeed();
  }else{
    // TODO: auto logic hier invullen
  }
